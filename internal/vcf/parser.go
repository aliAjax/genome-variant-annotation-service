...
func (p *Parser) Parse(r io.Reader) (Result, error) {
	...
	if err := scanner.Err(); err != nil {
		return Result{}, fmt.Errorf("scan vcf: %w", err)
	}
...
func parseRecord(line string, lineNo int) (variant.Variant, error) {
	...
	if err != nil {
		return variant.Variant{}, fmt.Errorf("line %d position: %w", lineNo, err)
	}
	var quality *float64
	if fields[5] != "." {
		v, err := strconv.ParseFloat(fields[5], 64)
		if err != nil {
			return variant.Variant{}, fmt.Errorf("line %d quality: %w", lineNo, err)
		}
		quality = &v
	}
	filters := []string{}
